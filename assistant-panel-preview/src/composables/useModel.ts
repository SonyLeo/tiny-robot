...
function createModelStore() {
  ...
  const firstAvailableModel = modelOptions.find((item) => item.apiKey) || modelOptions[0]
  const selectedModelId = ref(firstAvailableModel?.id || '')
  ...
  const availableModelCount = computed(() => modelOptions.filter((item) => item.apiKey).length)
