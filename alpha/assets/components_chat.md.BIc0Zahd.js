...
`,w=JSON.parse('{"title":"Chat 接入与入口","description":"","frontmatter":{"outline":[2,3]},"headers":[],"relativePath":"components/chat.md","filePath":"components/chat.md"}'),v={name:"components/chat.md"},_=Object.assign(v,{setup(x){const l=B();c(async()=>{l.value=(await p(async()=>{const{default:a}=await import("./chunks/preset-overrides.DPEAM2iQ.js");return{default:a}},__vite__mapDeps([0,1,2,3,4,5,6]))).default});const o=b(!0),d=B();return c(async()=>{d.value=(await p(async()=>{const{default:a}=await import("./chunks/blackbox.D6s9h1xm.js");return{default:a}},__vite__mapDeps([7,1,2,3,4,5,6]))).default}),(a,e)=>{const r=f("ClientOnly");return g(),y("div",null,[e[2]||(e[2]=s(`<h1 id="chat-接入与入口" tabindex="-1">Chat 接入与入口 <a class="header-anchor" href="#chat-接入与入口" aria-label="Permalink to &quot;Chat 接入与入口&quot;">​</a></h1><p><code>@opentiny/tiny-robot-chat</code> 的主入口是 <code>TrChat</code>。</p><p>这页只解决 3 件事：</p><ul><li>先把一个完整聊天页跑起来</li><li>先分清 <code>config / runtime / callbacks / presetOverrides</code></li><li>先判断自己下一步应该看配置页，还是进阶页</li></ul><blockquote><p>本页示例统一请求 <code>/api/chat/completions</code>。在文档站内，service worker 会接管这个请求并返回 mock 响应；接入业务时，请替换成你自己的服务端 API。</p></blockquote><h2 id="什么时候看这页" tabindex="-1">什么时候看这页 <a class="header-anchor" href="#什么时候看这页" aria-label="Permalink to &quot;什么时候看这页&quot;">​</a></h2><p>适合：</p><ul><li>第一次接入 <code>@opentiny/tiny-robot-chat</code></li><li>想先用默认页面跑通一个可用聊天页</li><li>还在判断 <code>TrChat</code> 本身够不够用</li></ul><p>如果你现在更关心：</p><ul><li>配置字段和能力开关写在哪里 <ul><li>去看 <a href="./chat-features.html">Chat 配置与能力</a></li></ul></li><li>如何定制结构、扩展运行时、接 MCP <ul><li>去看 <a href="./chat-advanced.html">Chat 定制与进阶</a></li></ul></li></ul><h2 id="先看这-3-条路径" tabindex="-1">先看这 3 条路径 <a class="header-anchor" href="#先看这-3-条路径" aria-label="Permalink to &quot;先看这 3 条路径&quot;">​</a></h2><table tabindex="0"><thead><tr><th style="text-align:left;">你要做什么</th><th style="text-align:left;">推荐入口</th><th style="text-align:left;">什么时候升级</th></tr></thead><tbody><tr><td style="text-align:left;">跑通一个完整聊天页</td><td style="text-align:left;"><code>TrChat</code></td><td style="text-align:left;">默认结构不够用再看进阶页</td></tr><tr><td style="text-align:left;">只做页面轻量差异</td><td style="text-align:left;"><code>presetOverrides</code></td><td style="text-align:left;">覆盖不够再用 <code>slots</code></td></tr><tr><td style="text-align:left;">默认结构不够用</td><td style="text-align:left;"><code>TrChat.Scaffold</code> / <code>TrChat.Provider</code></td><td style="text-align:left;">需要自己接管运行时或页面装配时再进入</td></tr></tbody></table><p>一个简单判断顺序：</p><ol><li>先用 <code>TrChat</code></li><li>再用 <code>presetOverrides</code></li><li>再用 <code>slots</code></li><li>最后再考虑 <code>Scaffold</code> 或 <code>Provider</code></li></ol><h2 id="开始前先确认-3-个前提" tabindex="-1">开始前先确认 3 个前提 <a class="header-anchor" href="#开始前先确认-3-个前提" aria-label="Permalink to &quot;开始前先确认 3 个前提&quot;">​</a></h2><p>如果你是第一次在业务项目里接入 <code>@opentiny/tiny-robot-chat</code>，建议先把下面 3 件事补齐，再去复制最小示例。</p><h3 id="_1-先安装运行这个最小示例需要的依赖" tabindex="-1">1. 先安装运行这个最小示例需要的依赖 <a class="header-anchor" href="#_1-先安装运行这个最小示例需要的依赖" aria-label="Permalink to &quot;1. 先安装运行这个最小示例需要的依赖&quot;">​</a></h3><p><code>@opentiny/tiny-robot-chat</code> 不是一个完全独立的单包入口。按当前包定义和 demo 工程的真实依赖，最少需要先安装：</p><ul><li><code>vue</code></li><li><code>@opentiny/tiny-robot</code></li><li><code>@opentiny/tiny-robot-chat</code></li><li><code>@opentiny/tiny-robot-kit</code></li><li><code>markstream-vue</code></li></ul><div class="vp-code-group vp-adaptive-theme"><div class="tabs"><input type="radio" name="group-aHkbJ" id="tab-ZoxJgZ0" checked><label data-title="pnpm" for="tab-ZoxJgZ0">pnpm</label><input type="radio" name="group-aHkbJ" id="tab-SCD8ST1"><label data-title="yarn" for="tab-SCD8ST1">yarn</label><input type="radio" name="group-aHkbJ" id="tab-I5IBrQC"><label data-title="npm" for="tab-I5IBrQC">npm</label></div><div class="blocks"><div class="language-bash vp-adaptive-theme active"><button title="Copy Code" class="copy"></button><span class="lang">bash</span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span style="--shiki-light:#6F42C1;--shiki-dark:#B392F0;">pnpm</span><span style="--shiki-light:#032F62;--shiki-dark:#9ECBFF;"> add</span><span style="--shiki-light:#032F62;--shiki-dark:#9ECBFF;"> vue</span><span style="--shiki-light:#032F62;--shiki-dark:#9ECBFF;"> @opentiny/tiny-robot</span><span style="--shiki-light:#032F62;--shiki-dark:#9ECBFF;"> @opentiny/tiny-robot-chat</span><span style="--shiki-light:#032F62;--shiki-dark:#9ECBFF;"> @opentiny/tiny-robot-kit</span><span style="--shiki-light:#032F62;--shiki-dark:#9ECBFF;"> markstream-vue</span></span></code></pre></div><div class="language-bash vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">bash</span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span style="--shiki-light:#6F42C1;--shiki-dark:#B392F0;">yarn</span><span style="--shiki-light:#032F62;--shiki-dark:#9ECBFF;"> add</span><span style="--shiki-light:#032F62;--shiki-dark:#9ECBFF;"> vue</span><span style="--shiki-light:#032F62;--shiki-dark:#9ECBFF;"> @opentiny/tiny-robot</span><span style="--shiki-light:#032F62;--shiki-dark:#9ECBFF;"> @opentiny/tiny-robot-chat</span><span style="--shiki-light:#032F62;--shiki-dark:#9ECBFF;"> @opentiny/tiny-robot-kit</span><span style="--shiki-light:#032F62;--shiki-dark:#9ECBFF;"> markstream-vue</span></span></code></pre></div><div class="language-bash vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">bash</span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span style="--shiki-light:#6F42C1;--shiki-dark:#B392F0;">npm</span><span style="--shiki-light:#032F62;--shiki-dark:#9ECBFF;"> install</span><span style="--shiki-light:#032F62;--shiki-dark:#9ECBFF;"> vue</span><span style="--shiki-light:#032F62;--shiki-dark:#9ECBFF;"> @opentiny/tiny-robot</span><span style="--shiki-light:#032F62;--shiki-dark:#9ECBFF;"> @opentiny/tiny-robot-chat</span><span style="--shiki-light:#032F62;--shiki-dark:#9ECBFF;"> @opentiny/tiny-robot-kit</span><span style="--shiki-light:#032F62;--shiki-dark:#9ECBFF;"> markstream-vue</span></span></code></pre></div></div></div><p>如果你只安装 <code>@opentiny/tiny-robot-chat</code>，照着下面的最小示例直接复制，通常并不能完整跑起来。</p><h3 id="_2-在入口文件引入基础样式" tabindex="-1">2. 在入口文件引入基础样式 <a class="header-anchor" href="#_2-在入口文件引入基础样式" aria-label="Permalink to &quot;2. 在入口文件引入基础样式&quot;">​</a></h3><p>至少先在 <code>main.ts</code> / <code>main.js</code> 里引入 TinyRobot 的基础样式：</p><div class="language-ts vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">ts</span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span style="--shiki-light:#D73A49;--shiki-dark:#F97583;">import</span><span style="--shiki-light:#24292E;--shiki-dark:#E1E4E8;"> { createApp } </span><span style="--shiki-light:#D73A49;--shiki-dark:#F97583;">from</span><span style="--shiki-light:#032F62;--shiki-dark:#9ECBFF;"> &#39;vue&#39;</span></span>
