...
| --- | --- | --- |
| `header` | 默认标题栏 | 替换顶部区域 |
| `left-aside` | `TrHistory` | 替换会话列表区域 |
| `main` | `TrBubbleProvider + TrBubbleList / TrWelcome / TrPrompts` | 替换消息区域 |
| `footer` | `TrSender` | 替换输入区域 |
...
- 不导出 `TrChat.Root / TrChat.Messages / TrChat.Composer / TrChat.Conversations` 作为 v1 稳定入口。
