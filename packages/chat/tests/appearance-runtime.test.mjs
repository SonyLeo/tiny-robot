...
const chatLayoutPath = fileURLToPath(new URL('../src/components/chat/ChatLayout.vue', import.meta.url))
...
await runTest('ChatLayout source bridges appearance mode through ThemeProvider instead of writing color-mode attr directly', async () => {
  assert.equal(chatLayoutSource.includes('ThemeProvider'), true)
...
await runTest('ChatLayout source keeps system appearance mapping for runtime color-mode resolution', async () => {
  assert.equal(chatLayoutSource.includes("mode === 'system'"), true)
  assert.equal(chatLayoutSource.includes("'auto'"), true)
})

await runTest('ChatLayout source keeps explicit light/dark appearance mapping for runtime color-mode resolution', async () => {
  assert.equal(chatLayoutSource.includes("mode === 'light'"), true)
  assert.equal(chatLayoutSource.includes("mode === 'dark'"), true)
})
