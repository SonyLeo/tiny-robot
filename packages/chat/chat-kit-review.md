...
| 编号 | 优化项 | 价值 | 计划阶段 |
|:-----|:-------|:-----|:---------|
| B1 | `useChatKit` 状态分片 | 降低 composable 膨胀，提升测试性 | 已完成（本轮） |
| B2 | 消息乐观更新 + 错误回滚 | 提升发送反馈速度和失败体验 | 已完成（本轮） |
| B3 | 消息操作统一回调入口 | 统一埋点与业务扩展入口 | 已完成（本轮） |
| B4 | 结构化错误 + 重试机制 | 提升生产可用性和恢复能力 | 已完成（本轮） |
| B5 | ChatList `docs` variant | 扩展知识库 / 文档问答场景 | 已完成（本轮） |
...
| 编号 | 事项 | 对应价值 |
|:-----|:-----|:---------|
| B1 | `useChatKit` 状态分片 | 已完成（可维护性） |
| B2 | 乐观更新 + 回滚 | 已完成（交互体验） |
| B3 | 统一消息操作入口 | 已完成（扩展性） |
| B4 | 结构化错误 + 重试 | 已完成（稳定性） |
| B5 | `docs` variant | 已完成（场景覆盖） |
...
> 本文最初用于评估与规划 `packages/chat` 的架构整改和能力演进。  
> 截至 `2026-03-15`，本轮实施范围已经完成，下面这份评审文档应按“已落地结论 + 后续增强参考”来阅读，而不是纯待办列表。
