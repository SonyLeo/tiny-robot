...
import { createAnchorTestHelper } from './testHelper'

test.describe('Anchor component e2e', () => {
  test.describe.configure({ mode: 'serial' })

  const helperFactory = createAnchorTestHelper

  test.beforeEach(async ({ page }) => {
    const helper = helperFactory(page)
    await helper.gotoDemo()

    const ready = await helper.isAnchorReady()
    test.skip(!ready, 'TrAnchor is not exported yet in @opentiny/tiny-robot.')
  })
  ...
  test('focus alone does not show tooltip for truncated items', async ({ page }) => {
    ...
    const longItemButton = page
      .locator(helper.selectors.anchorRoot)
  ...
  test('hovering a new item clears the previous tooltip even if the old item keeps focus', async ({ page }) => {
    ...
    await page.waitForTimeout(320)
    await helper.expectTooltipVisibleForLabel('Postmortem draft notes with extended follow-up context', false)
    await expect(page.locator(`${helper.selectors.anchorRoot} .tr-anchor__list-item.is-tooltip-visible`)).toHaveCount(0)
  })
  ...
  test('search query model changes and list can be filtered by assistant reply text', async ({ page }) => {
    ...
    await expect(page.locator(helper.selectors.anchorRoot)).toContainText('Incident timeline planning')
    await expect(page.locator(helper.selectors.anchorRoot)).not.toContainText('Project kickoff summary')
  })
  ...
  test('highlighted item resyncs to the active item after clearing a filtered search query', async ({ page }) => {
    ...
    await helper.setExternalQuery('retry storms')
    await helper.hoverNav()
    await expect(page.locator(`${helper.selectors.anchorRoot} [data-item-id="turn-2"]`)).toHaveAttribute(
      'tabindex',
      '0',
    )
    ...
    await page.locator(helper.selectors.anchorOverlay).evaluate((el) => {
  ...
  test('clicking item applies and clears the target active class automatically', async ({ page }) => {
    const helper = helperFactory(page)
    const target = page.locator('[data-anchor-id="turn-5"]').first()

    await helper.resetState()
    await expect(target).not.toHaveClass(/tr-anchor-target--flash/)

    await helper.clickNavItemByLabel('Release train dependencies')

    await expect(target).toHaveClass(/tr-anchor-target--flash/)
    await expect.poll(async () => (await target.getAttribute('class')) ?? '').not.toContain('tr-anchor-target--flash')
  })
  ...
  test('bubble scene resolves marked box nodes as scroll targets', async ({ page }) => {
    ...
    const bubbleTarget = page.locator('[data-anchor-id="turn-5"]').first()
  ...
  test('bubble scene keeps the current turn active until the next turn reaches the configured offset', async ({
    ...
    await page.locator(helper.selectors.scrollContainer).evaluate((container) => {
      ...
      const nextTurn = root.querySelector('[data-anchor-id="turn-4"]') as HTMLElement | null
      if (!nextTurn) {
  ...
  test('shows empty state when the controlled search query has no matches', async ({ page }) => {
    ...
    await expect(page.locator(helper.selectors.anchorRoot)).toContainText('No matching items')
  ...
  test('treats empty matcher arrays as non-matches instead of blank rows', async ({ page }) => {
    ...
    await expect(page.locator(helper.selectors.anchorRoot)).toContainText('No matching items')
    await expect(page.locator(`${helper.selectors.anchorRoot} .tr-anchor__item`)).toHaveCount(0)
  })
