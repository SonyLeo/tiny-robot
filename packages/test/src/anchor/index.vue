<template>
  <div class="anchor-demo">
    <h2 data-testid="anchor-page-title">Anchor component test</h2>
    <p class="availability">
      Component export status:
      <strong data-testid="anchor-availability">{{ hasAnchor ? 'ready' : 'missing' }}</strong>
    </p>
    ...
    <div class="controls">
      ...
      <button data-testid="reset-anchor-state" class="reset-btn" @click="resetState">Reset state</button>
    </div>
    ...
    <div class="workspace" :class="{ 'is-document-scroll': documentScrollMode }">
      <div class="anchor-host" data-testid="anchor-host">
        <component
          :is="resolvedAnchor"
          ...
          :tooltip-delay="260"
          target-feedback-class="tr-anchor-target--flash"
...
import type { AnchorItem, BubbleBoxRendererMatch, BubbleMessage, BubbleRoleConfig } from '@opentiny/tiny-robot'
...
const FallbackAnchor = defineComponent({
  name: 'TrAnchorFallback',
  template: '<div class="anchor-fallback" data-testid="anchor-fallback">TrAnchor is not exported yet.</div>',
})
...
function emptyArrayMatcher(item: AnchorItem, rawQuery: string) {
    ...
    ? [
        {
          ...
          searchText: 'An anchor item without any matching DOM target.',
...
const hasAnchor = computed(() => Boolean((TinyRobot as Record<string, unknown>).TrAnchor))
...
const resolvedAnchor = computed<Component>(() => {
  const maybeComponent = (TinyRobot as Record<string, unknown>).TrAnchor as Component | undefined
  return maybeComponent ?? FallbackAnchor
})
...
.workspace.is-document-scroll .anchor-host {
  position: sticky;
  top: 16px;
