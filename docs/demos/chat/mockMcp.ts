...
export const MOCK_PLUGINS: PluginInfo[] = [
  {
    ...
    enabled: true,
    tools: [
      { id: 'get_current_time', name: 'get_current_time', description: '获取当前时间和日期', enabled: true },
      { id: 'calculate', name: 'calculate', description: '执行数学计算', enabled: true },
      { id: 'word_count', name: 'word_count', description: '统计文本的字数和字符数', enabled: true },
    ],
  },
  {
    id: 'weather',
    ...
    enabled: true,
    tools: [{ id: 'get_weather', name: 'get_weather', description: '查询指定城市的天气信息', enabled: true }],
  },
]
    ...
    return [
      {
        ...
        function: {
          ...
          parameters: {
            type: 'object',
            properties: {
              timezone: { type: 'string', description: '时区，如 Asia/Shanghai，默认为本地时区' },
              format: {
                type: 'string',
                enum: ['full', 'date', 'time'],
