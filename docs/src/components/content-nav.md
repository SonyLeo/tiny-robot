...
1. 目录项 `id` 与目标节点上的 `data-content-nav-id` 保持一致。
...
| 属性 | 类型 | 默认值 | 说明 |
| --- | --- | --- | --- |
| `items` | [`ContentNavItem[]`](#content-nav-item) | - | 目录项列表。组件默认使用 `item.id` 匹配 `[data-content-nav-id="<id>"]`，并滚动到对应节点 |
...
| `searchOptions` | [`ContentNavSearchOptions`](#content-nav-search-options) | - | 搜索区配置。不传时不显示搜索区 |
...
| `select` | item: [`ContentNavItem`](#content-nav-item) | 目录项被选中时触发 |

## Types

### ContentNavItem {#content-nav-item}
...
### ContentNavSearchOptions {#content-nav-search-options}

| 字段 | 类型 | 说明 |
| --- | --- | --- |
| `placeholder` | `string` | 搜索框占位文案 |
| `matcher` | [`ContentNavSearchMatcher`](#content-nav-search-matcher) | 自定义搜索匹配逻辑 |
| `clearOnCollapse` | `boolean` | 收起时是否清空搜索词 |

### ContentNavSearchMatcher {#content-nav-search-matcher}

```ts
type ContentNavSearchMatcher = (
  item: ContentNavItem,
  searchQuery: string,
