...
function createChatStore() {
  ...
  function syncConversationTitle(content: string) {
    ...
    const hasVisibleMessages = activeConversation.engine.messages.value.some((item) => item.role !== 'system')
    if (!hasVisibleMessages || activeConversation.title === DEFAULT_CONVERSATION_TITLE) {
